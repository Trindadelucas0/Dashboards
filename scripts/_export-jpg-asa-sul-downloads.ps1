# Export Downloads ASA SUL Jan-Jul acumulado
...
function Export-Movimento($excel, $path, $key, $tipo) {
  ...
  $payload = [ordered]@{
    ...
    unidade='ASA_SUL'; modo='acumulado'; competencia=$null;
...
$jobs = @(
  @{ key='asa-sul-janjul-entradas'; path='c:\Users\trind\Downloads\ENTRADA FILIAL DF ASA SUL.xls'; tipo='entradas' },
  @{ key='asa-sul-janjul-saidas'; path='c:\Users\trind\Downloads\SAIDA FILIAL DF ASA SUL.xls'; tipo='saidas' }
)
...
try {
  foreach ($job in $jobs) {
    ...
    Export-Movimento $excel $job.path $job.key $job.tipo
  }
}
finally {
  $excel.Quit()
