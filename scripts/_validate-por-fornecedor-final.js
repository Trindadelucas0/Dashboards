...
function checkView(t) {
  ...
  const need = [
    ...
    ["excelInModal", /id=["']supplierModal["'][\s\S]*?btn-xlsx-supplier[\s\S]*?<\/div>\s*<\/div>/.test(html)],
    ["footerActions", /<div class="modal-footer-actions">/.test(html)],
    ["printBtnClass", /btn-print-supplier/.test(html)],
    ["xlsxCss", /#supplierModal\s+\.btn-confirm\.btn-xlsx-supplier/.test(html)],
    ["noInlineXlsxBg", !/btn-xlsx-supplier[^>]*style=["'][^"']*background/i.test(html)],
