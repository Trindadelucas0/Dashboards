'use strict';
/**
 * Remove duplicação MATRIZ↔ASA_SUL, recalcula DRE (sem remessas) e patcha renderDRE.
 */
const fs = require('fs');
...
function buildEmpresa(filiais, mesKey) {
  ...
  const dre = buildDreFromPack({ kpis, cfop_saidas, cfop_entradas });
  return {
    meta: {
      codigo: 'TODAS',
      nome: 'JPG - PRODUTOS FUNCIONAIS E NUTRICIONAIS',
...
// Patch renderDRE UI
const oldRender = `function renderDRE(D, pfx) {
  const d = D.dre;
...
const newRender = `function renderDRE(D, pfx) {
  const d = D.dre;
...
if (!html.includes(oldRender)) {
  console.error('renderDRE block not found for replace');
  process.exit(2);
}
...
for (const mes of Object.keys(data.fiscalPorMes.porMes).sort()) {
  ...
  // Prefer ASA_SUL pack; if MATRIZ still holds the same totals, clear MATRIZ
  ...
  console.log(mes, 'ext=' + d.receita_externa, 'rem=' + d.remessas_transferencias, 'lucro=' + d.lucro_bruto, 'margem=' + d.margem_bruta_pct, 'consolS=' + month.empresa.kpis.saidas);
}

data.meta.fonte = 'Relatórios ICMS (.xls) — Filial Asa Sul DF Jan–Jul (DRE sem remessas; sem duplicar Filial DF)';
...
let ok = true;
if (!checkHtml.includes("filial_key === 'ASA_SUL'")) {
  console.error('UI patch missing');
  ok = false;
}
for (const mes of Object.keys(check.fiscalPorMes.porMes).sort()) {
  ...
  if (!near(mz.kpis.entradas, 0) || !near(mz.kpis.saidas, 0)) {
    console.error('MATRIZ not empty', mes); ok = false;
  }
  const rem = sumCfop(a.cfop_saidas, isRemessaCfop);
  if (!near(a.dre.receita_externa, round2(a.kpis.saidas - rem))) {
    console.error('ext fail', mes); ok = false;
  }
  if (!near(a.dre.lucro_bruto, round2(a.dre.receita_externa - a.dre.cmv))) {
    console.error('lucro fail', mes); ok = false;
  }
  ...
  if (!near(m.empresa.kpis.entradas, expectE) || !near(m.empresa.kpis.saidas, expectS)) {
    console.error('consol fail', mes, m.empresa.kpis, expectE, expectS); ok = false;
  }
  // no double: consol saidas must be < 2 * asa saidas + others rough check
  if (near(m.empresa.kpis.saidas, round2(a.kpis.saidas * 2 + expectS - a.kpis.saidas))) {
    // if somehow still double this would match oddly; skip
  }
}
console.log(ok ? 'DRE+DEDUP OK' : 'DRE+DEDUP FAILED');
