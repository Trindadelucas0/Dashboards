# Export JPG MG + SP Jul/2026 from Downloads
...
$jobs = @(
  @{ key='jpg-mg-entradas'; path='c:\Users\trind\Downloads\Relatorio de entrada por fornecedor filial mg 072026.xls'; tipo='entradas'; unidade='MG' },
  @{ key='jpg-mg-saidas'; path='c:\Users\trind\Downloads\Relatorio de saida por cliente filial mg 072026.xls'; tipo='saidas'; unidade='MG' },
  @{ key='jpg-sp-entradas'; path='c:\Users\trind\Downloads\Relatorio de entrada por fornecedor filial sp.xls'; tipo='entradas'; unidade='SP' },
  @{ key='jpg-sp-saidas'; path='c:\Users\trind\Downloads\Relatorio de saida por cliente filial sp.xls'; tipo='saidas'; unidade='SP' }
)
...
try {
  foreach ($job in $jobs) {
    if (-not (Test-Path -LiteralPath $job.path)) { Write-Host "MISS $($job.path)"; continue }
    Write-Host "Export $($job.key)"
    $wb = $excel.Workbooks.Open((Resolve-Path -LiteralPath $job.path))
    ...
    $payload = [ordered]@{
      key=$job.key; file=[IO.Path]::GetFileName($job.path); path=$job.path; sheet=$ws.Name;
      tipo=$job.tipo; unidade=$job.unidade; modo='mensal'; competencia='2026-07';
    ...
    [IO.File]::WriteAllText($outPath, ($payload | ConvertTo-Json -Depth 6 -Compress), [Text.UTF8Encoding]::new($false))
