# Export SEDE (Matriz Sede / 0001-65) Jul/2026 entradas — layout raiz (valor col 21)
$ErrorActionPreference = 'Stop'
$root = Resolve-Path (Join-Path $PSScriptRoot '..')
$outDir = Join-Path $root 'relatorios\2026-07\raw'
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

$path = 'c:\Users\trind\Downloads\Relatorio de entrada matriz.xls'
$key = 'jpg-sede-entradas'

function Get-CellText($ws, $r, $c) {
  $t = $ws.Cells.Item($r, $c).Text
  if ($null -eq $t) { return '' }
  return ([string]$t).Trim()
}
function Get-CellNum($ws, $r, $c) {
  $v = $ws.Cells.Item($r, $c).Value2
  if ($null -eq $v) { return $null }
  if ($v -is [double] -or $v -is [decimal] -or $v -is [int] -or $v -is [long]) { return [double]$v }
  $s = ([string]$v).Trim()
  if (-not $s) { return $null }
  if ($s -match '^\d{1,3}(\.\d{3})*,\d+$') {
    $s = $s.Replace('.', '').Replace(',', '.')
    return [double]$s
  }
  $parsed = 0.0
  if ([double]::TryParse($s, [ref]$parsed)) { return $parsed }
  return $null
}
function Format-Cfop($rawText, $rawVal) {
  $t = if ($rawText) { $rawText.Trim() } else { '' }
  if ($t -match '^(\d)-(\d{3})$') { return $t }
  if ($t -match '^(\d)\.(\d{3})$') { return "$($Matches[1])-$($Matches[2])" }
  if ($t -match '^(\d)(\d{3})$') { return "$($Matches[1])-$($Matches[2])" }
  if ($null -ne $rawVal) {
    $n = [int][Math]::Round([double]$rawVal)
    $s = $n.ToString()
    if ($s.Length -eq 4) { return "$($s.Substring(0,1))-$($s.Substring(1))" }
  }
  return $t
}
function Split-Fornecedor($raw) {
  $s = ([string]$raw).Trim()
  if ($s -match '^([\d.\-\/]+)\s+(.+)$') {
    return @{ doc = $Matches[1]; nome = $Matches[2].Trim() }
  }
  return @{ doc = ''; nome = $s }
}

$excel = New-Object -ComObject Excel.Application
$excel.Visible = $false
$excel.DisplayAlerts = $false
$excel.ScreenUpdating = $false

try {
  Write-Host "Export $key"
  $wb = $excel.Workbooks.Open((Resolve-Path -LiteralPath $path))
  $ws = $wb.Worksheets.Item(1)
  $used = $ws.UsedRange
  $rows = $used.Rows.Count
  $cols = $used.Columns.Count
  $company = Get-CellText $ws 1 1
  $cnpj = Get-CellText $ws 2 3
  if (-not $cnpj) { $cnpj = Get-CellText $ws 2 4 }
  $period = Get-CellText $ws 4 3
  if (-not $period) { $period = Get-CellText $ws 4 4 }
  $ie = Get-CellText $ws 3 3
  if (-not $ie) { $ie = Get-CellText $ws 3 4 }

  $lines = New-Object System.Collections.Generic.List[object]
  $totalGeral = $null
  $totalIcms = 0.0
  $totalIpi = 0.0
  $last = $null

  for ($r = 6; $r -le $rows; $r++) {
    $c1 = Get-CellText $ws $r 1
    if ($c1 -match '^Total Geral') {
      for ($tr = $r; $tr -le [Math]::Min($r + 4, $rows); $tr++) {
        $n = Get-CellNum $ws $tr 21
        if ($null -ne $n -and $n -gt 0 -and $null -eq $totalGeral) { $totalGeral = $n }
        $tax = Get-CellText $ws $tr 24
        $tv = Get-CellNum $ws $tr 29
        if ($tax -match 'ICMS' -and $null -ne $tv) { $totalIcms = $tv }
        if ($tax -match 'IPI' -and $null -ne $tv) { $totalIpi = $tv }
      }
      break
    }
    if ($c1 -match '^Total Fornecedor') { continue }
    if ($c1 -match '^ACOMPANHAMENTO' -or $c1 -match '^C.?digo' -or $c1 -match '^Sistema') { continue }

    if (-not $c1 -or $c1 -notmatch '^\d+$') {
      $tax = Get-CellText $ws $r 24
      if ($last -and $tax -match 'IPI') {
        $ipi = Get-CellNum $ws $r 29
        if ($null -ne $ipi) { $last.ipi = [Math]::Round($ipi, 2) }
      }
      continue
    }

    $data = Get-CellText $ws $r 3
    $nota = Get-CellText $ws $r 5
    $serie = Get-CellText $ws $r 7
    $forn = Split-Fornecedor (Get-CellText $ws $r 13)
    $cfopText = Get-CellText $ws $r 15
    $cfopVal = $ws.Cells.Item($r, 15).Value2
    $uf = Get-CellText $ws $r 19
    $valor = Get-CellNum $ws $r 21
    $tax = Get-CellText $ws $r 24
    $base = Get-CellNum $ws $r 26
    $imposto = Get-CellNum $ws $r 29

    if ($null -eq $valor) { continue }
    $cfop = Format-Cfop $cfopText $cfopVal
    if (-not $cfop) { continue }

    $icms = 0.0; $ipi = 0.0
    if ($tax -match 'ICMS' -and $null -ne $imposto) { $icms = [Math]::Round($imposto, 2) }
    if ($tax -match 'IPI' -and $null -ne $imposto) { $ipi = [Math]::Round($imposto, 2) }

    $obj = [ordered]@{
      codigo = $c1; data = $data; nota = $nota; serie = $serie
      nome = $forn.nome; doc = $forn.doc; uf = $uf
      cfop = $cfop; valor = [Math]::Round($valor, 2)
      base = $(if ($null -ne $base) { [Math]::Round($base, 2) } else { 0 })
      icms = $icms; ipi = $ipi
    }
    $lines.Add($obj) | Out-Null
    $last = $obj
  }

  $payload = [ordered]@{
    key = $key; file = [IO.Path]::GetFileName($path); path = $path
    sheet = $ws.Name; tipo = 'entradas'; unidade = 'SEDE'
    modo = 'mensal'; competencia = '2026-07'
    company = $company; cnpj = $cnpj; ie = $ie; period = $period
    totalGeral = $totalGeral; totalIcms = $totalIcms; totalIpi = $totalIpi
    lineCount = $lines.Count; lines = $lines
    layout = 'raiz-entradas-col21'
  }
  $outPath = Join-Path $outDir ($key + '.json')
  [System.IO.File]::WriteAllText($outPath, ($payload | ConvertTo-Json -Depth 6 -Compress), [System.Text.UTF8Encoding]::new($false))
  Write-Host ("  lines={0} total={1} period={2} cnpj={3}" -f $lines.Count, $totalGeral, $period, $cnpj)
  $wb.Close($false)
}
finally {
  $excel.Quit()
  [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null
  [GC]::Collect(); [GC]::WaitForPendingFinalizers()
}
Write-Host 'DONE'
